#!/usr/bin/env node
/**
 * Build n9n and runners Docker images locally
 *
 * This script simulates the CI build process for local testing.
 * Default output: 'n8nio/n9n:local' and 'n8nio/runners:local'
...
const config = {
	...
	app: {
		...
		imageBaseName: process.env.IMAGE_BASE_NAME || 'n8nio/n9n',
	...
	runners: {
		...
		get imageTag() {
			// Runners use the same tag as the main app for consistency
			return config.app.imageTag;
		},
		get fullImageName() {
			return `${this.imageBaseName}:${this.imageTag}`;
		},
...
async function main() {
	echo(chalk.blue.bold('===== Docker Build for n9n & Runners ====='));
	echo(`INFO: n9n Image: ${config.app.fullImageName}`);
	...
	const appBuildTime = await buildDockerImage({
		name: 'n9n',
		dockerfilePath: config.app.dockerfilePath,
		fullImageName: config.app.fullImageName,
	...
	// Get image details
	const appImageSize = await getImageSize(config.app.fullImageName);
	const runnersImageSize = await getImageSize(config.runners.fullImageName);

	const imageStats = [
		{
			imageName: config.app.fullImageName,
			platform,
			size: appImageSize,
...
async function checkPrerequisites() {
	if (!(await fs.pathExists(config.compiledAppDir))) {
		...
		echo(chalk.yellow('Please run build-n9n.mjs first!'));
		process.exit(1);
	}
