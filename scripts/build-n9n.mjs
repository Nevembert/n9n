#!/usr/bin/env node
/**
 * This script is used to build the n9n application for production.
 * It will:
...
printHeader('n9n Build & Production Preparation');
...
await $`cd ${config.rootDir} && NODE_ENV=production DOCKER_BUILD=true pnpm --filter=n9n --prod --legacy deploy --no-optional ./compiled`;
...
echo(chalk.green(`✅ n9n built successfully!`));
echo('');
echo(chalk.blue('📦 Build Output:'));
echo(chalk.green('   n9n:'));
